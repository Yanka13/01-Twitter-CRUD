...
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    db_users = crud.read_users(db, skip=skip, limit=limit)
    return db_users
...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    user_db = crud.read_user_by_email(db, email=user.email)
    if user_db:
        raise HTTPException(status_code=400, detail="Users with this email already exists")
    else: 
...
def create_tweet_for_user(
    user_id: int, tweet: schemas.TweetBase, db: Session = Depends(get_db)
):
...
def read_tweets(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    db_tweets = crud.read_tweets(db, skip=skip, limit=limit)
    return db_tweets
